...
export default function UsersPage() {
  ...
  const getBadge = (tag: any) => {
    switch (tag) {
      case "owner":
        return { icon: ShieldCheck, color: "text-indigo-600", label: "Owner" };
      case "developer":
        return { icon: Code2, color: "text-purple-500", label: "Developer" };
      case "manager":
        return { icon: Briefcase, color: "text-green-600", label: "Manager" };
      case "premium":
        return { icon: BadgeCheck, color: "text-yellow-600", label: "Premium" };
      case "user":
  ...
  const getTagInfoText = (tag: string) => {
    const key = (tag || "").toLowerCase();
    switch (key) {
      case "owner":
        return "This is an owner tag. Full administrative privileges.";
      case "developer":
        return "This is a developer tag. Reserved for maintainers; cannot be assigned here.";
      case "manager":
        return "This is a manager tag. Elevated permissions for user/batch management.";
      case "premium":
        return "This is a premium tag. Give Premium access to user to skip verification.";
      case "user":
        return "This is a user tag. With this User need to verify shortlinks.";
      default:
        return "Select a tag to see its description.";
    }
  };

  const openChangeTagDialog = (user: User) => {
    const currentTag = (user.tag || "").toLowerCase();
    if (currentTag === "developer") {
      toast.error("Developer tags can't be changed");
      return;
    }
    setTagDialogUser(user);
    setSelectedTag(currentTag || "user");
    setIsTagDialogOpen(true);
  };

  const handleSaveUserTag = async () => {
    if (!tagDialogUser) return;
    try {
      setSavingTag(true);
      const res = await fetch("/api/admin/update-user-tag", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ userId: tagDialogUser._id, tag: selectedTag }),
      });
      if (!res.ok) throw new Error("Failed to update user tag");
      const data = await res.json();
      setUsers((prev) =>
        prev.map((u) =>
          u._id === tagDialogUser._id ? { ...u, tag: selectedTag } : u
        )
      );
      toast.success(`${tagDialogUser.UserName} promoted to ${selectedTag}`);
      setIsTagDialogOpen(false);
    } catch (e) {
      toast.error("Could not update user tag");
    } finally {
      setSavingTag(false);
    }
  };
    ...
    <AdminLayout activePage="users">
      <div className="p-3 lg:p-8">
        <Dialog open={isTagDialogOpen} onOpenChange={setIsTagDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Change User Tag</DialogTitle>
              <DialogDescription>
                Update the role tag for this user.
              </DialogDescription>
            </DialogHeader>
            {tagDialogUser && (
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <Avatar className="w-10 h-10">
                    <AvatarImage src={tagDialogUser.photoUrl || ""} />
                    <AvatarFallback>{getInitials(tagDialogUser.UserName)}</AvatarFallback>
                  </Avatar>
                  <div>
                    <div className="font-medium">{tagDialogUser.UserName}</div>
                    <div className="text-xs text-gray-500">{tagDialogUser.phoneNumber}</div>
                  </div>
                </div>
                ...
                <DialogFooter>
                  ...
                  <Button onClick={handleSaveUserTag} disabled={savingTag || selectedTag === "developer"}>
