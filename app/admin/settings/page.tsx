...
export default function AdminSettings() {
    ...
    const fetchConfig = async () => {
      try {
        ...
        setConfig({
          ...
          authEnabled: serverConfig.authEnabled === true,
          keyGenerationEnabled: serverConfig.keyGenerationEnabled === true,
          penpencilToken: "",
          penpencilRefreshToken: ""
        });

        setTokenMasked(serverConfig.penpencilTokenMasked || "");
        setTokenUpdatedAt(serverConfig.penpencilTokenUpdatedAt || null);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const saved = data.serverConfig || {};
        setTokenMasked(saved.penpencilTokenMasked || "");
        setTokenUpdatedAt(saved.penpencilTokenUpdatedAt || null);
        setConfig((prev) => ({ ...prev, penpencilToken: "", penpencilRefreshToken: "" }));
  ...
  const handleClearToken = async () => {
    if (!confirm("Remove the saved global PenPencil token?")) return;
    setLoading(true);
    try {
      const response = await fetch("/api/admin/adminServer", {
        method: "PUT",
        ...
        credentials: "include",
        body: JSON.stringify({ penpencilToken: null, penpencilRefreshToken: null }),
      });
      if (!response.ok) throw new Error("Failed");
      setTokenMasked("");
      setTokenUpdatedAt(null);
      setConfig((prev) => ({ ...prev, penpencilToken: "", penpencilRefreshToken: "" }));
      toast.success("Global token removed");
    } catch {
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Turn user login on or off for the whole website
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    ON: users must login (OTP / bot / direct login).{" "}
                    OFF: every visitor automatically gets a guest session and full access.
                  </p>
                </div>
                <Switch
                  checked={config.authEnabled}
                  onCheckedChange={(checked) =>
                    setConfig((prev) => ({ ...prev, authEnabled: checked }))
                  }
                />
              </div>
              ...
              >
                {config.authEnabled
                  ? "Auth is ON — normal login flow. Users use their own PenPencil tokens."
                  : "Auth is OFF — guest mode. Every visitor gets a unique guest session and all PenPencil data is fetched with the global token below."}
              </div>

              <div className="flex items-center justify-between gap-4">
                <div>
                  <Label className="text-sm font-medium text-gray-700">Key Generation</Label>
                  <p className="text-xs text-gray-500 mt-1">Allow the key-generate page</p>
                </div>
                <Switch
                  checked={config.keyGenerationEnabled}
                  onCheckedChange={(checked) =>
                    setConfig((prev) => ({ ...prev, keyGenerationEnabled: checked }))
                  }
                />
              </div>

              <p className="text-xs text-gray-500">
                Note: the admin panel always stays password protected.
              </p>
            </div>
          </Card>

          {/* Global PenPencil Token */}
          <Card className="p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-xl font-semibold text-gray-900">Global PenPencil Token</h2>
                <p className="text-sm text-gray-600">
                  Used by all guest users while authentication is OFF
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="penpencilToken" className="text-sm font-medium text-gray-700">
                  Access Token
                </Label>
                <Input
                  id="penpencilToken"
                  type="password"
                  autoComplete="off"
                  value={config.penpencilToken}
                  onChange={(e) => setConfig({ ...config, penpencilToken: e.target.value })}
                  placeholder={tokenMasked ? `Saved: ${tokenMasked} — paste a new token to replace` : "Paste PenPencil access token"}
                  className="mt-1 font-mono"
                />
                <p className="text-xs text-gray-500 mt-1">
                  {tokenMasked
                    ? `Currently saved: ${tokenMasked}${tokenUpdatedAt ? " • updated " + new Date(tokenUpdatedAt).toLocaleString() : ""}`
                    : "No global token saved yet — guests will not be able to load content."}
                </p>
              </div>

              <div>
                <Label htmlFor="penpencilRefreshToken" className="text-sm font-medium text-gray-700">
                  Refresh Token (optional)
                </Label>
                <Input
                  id="penpencilRefreshToken"
                  type="password"
                  autoComplete="off"
                  value={config.penpencilRefreshToken}
                  onChange={(e) =>
                    setConfig({ ...config, penpencilRefreshToken: e.target.value })
                  }
                  placeholder="Paste PenPencil refresh token"
                  className="mt-1 font-mono"
                />
              </div>
