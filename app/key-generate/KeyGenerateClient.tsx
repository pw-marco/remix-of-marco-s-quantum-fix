...
export default function KeyGenerateClient({
  ...
  useEffect(() => {
    // Set anon_id cookie on client side
    if (anon_id) {
                ...
                <div className="space-y-3">
                  {shortnerServers.map((server, idx) => (
                    <button
                      ...
                      className={`w-full group relative p-4 rounded-xl border transition-all duration-200 hover:scale-[1.01] focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-slate-900 ${server.shortenedUrl
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${server.shortenedUrl
                        ...
                        <div className="flex-1 text-left">
                          <h4 className={`font-semibold text-sm ${server.shortenedUrl ? 'text-white' : 'text-gray-400'
                            }`}>
                            {server.name}
                          </h4>
                          <p className={`text-xs mt-1 ${server.shortenedUrl ? 'text-gray-300' : 'text-gray-500'
                            }`}>
